...
class Checkpointer(Callback):
    def __init__(self, model):
        ...
        # A resumed run can carry forward the best score observed before an
        # interruption, even though Paddle's optimizer checkpoint does not
        # serialize callback state.
        self.best_ap = float(self.model.cfg.get('resume_best_ap', -1000.))
        if self.best_ap > -1000.:
            logger.info("Resume best bbox AP from {:.4f}.".format(
                self.best_ap))
