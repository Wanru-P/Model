...
class Multi_BatchRandomResize(BaseOperator):
    ...
    Args:
        ...
        random_size (bool): whether random select target size of image
        random_interp (bool): whether random select interpolation method
        pad_to_target (bool): after a keep-ratio resize, pad every modality to
            the selected target canvas. This is useful for synchronized
            RGB/IR letterbox training.
    """
    ...
    def __init__(self,
        ...
        self.random_size = random_size
        self.random_interp = random_interp
        self.pad_to_target = bool(pad_to_target)
        self.pad_mode = pad_mode
        self.fill_value = fill_value
        self.fill_value_ir = fill_value_ir
        self.return_pad_mask = bool(return_pad_mask)

        if self.pad_to_target and not self.keep_ratio:
            raise ValueError(
                'pad_to_target is only meaningful when keep_ratio=True.')
    ...
    def __call__(self, samples, context=None):
        ...
        resizer = Multi_Resize(
            target_size, keep_ratio=self.keep_ratio, interp=interp)
        samples = resizer(samples, context=context)

        if not self.pad_to_target:
            return samples

        if np.isscalar(target_size):
            pad_size = [int(target_size), int(target_size)]
        else:
            pad_size = list(target_size)
        padder = Multi_Pad_IRVIS(
            size=pad_size,
            pad_mode=self.pad_mode,
            fill_value=self.fill_value,
            fill_value_ir=self.fill_value_ir,
            return_pad_mask=self.return_pad_mask)
        return padder(samples, context=context)
