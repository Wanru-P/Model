...
class Multi_Decode(BaseOperator):
    def __init__(self, min_valid_depth=300.0, max_depth=20000.0,
                 log_depth=True):
    ...
    def _decode_depth(self, encoded):
        data = np.frombuffer(encoded, dtype='uint8')
        raw = cv2.imdecode(data, cv2.IMREAD_UNCHANGED)
        if raw is None:
            raise ValueError('Failed to decode depth image.')
        # Capture dtype before collapsing the three-channel JPG representation;
        # float conversion must not make the JPG branch look like metric PNG.
        is_uint8 = raw.dtype == np.uint8
        if raw.ndim == 3:
            raw = raw.astype(np.float32).mean(axis=2)
        raw = raw.astype(np.float32)
        if is_uint8:
            # The JPG subset is an already quantized depth representation.
            valid = (raw > 0.0).astype(np.float32)
            normalized = np.clip(raw, 0.0, 255.0) / 255.0
        else:
            valid = np.logical_and(
                raw >= self.min_valid_depth,
                raw <= self.max_depth).astype(np.float32)
            clipped = np.clip(raw, self.min_valid_depth, self.max_depth)
            if self.log_depth:
                low = np.log(self.min_valid_depth)
                high = np.log(self.max_depth)
                normalized = (np.log(clipped) - low) / (high - low)
            else:
                normalized = ((clipped - self.min_valid_depth) /
                              (self.max_depth - self.min_valid_depth))
        normalized *= valid
        return normalized[..., None], valid[..., None]
    ...
    def apply(self, sample, context=None):
        ...
        if 'image' not in sample:
            ...
            if 'depth_im_file' in sample:
                with open(sample['depth_im_file'], 'rb') as f:
                    sample['depth_image'] = f.read()
                sample.pop('depth_im_file')
        ...
        try:
            ...
            if 'depth_image' in sample:
                depth_im, depth_valid = self._decode_depth(
                    sample['depth_image'])
        except:
            vis_im = sample['vis_image']
            ir_im = sample['ir_image']
            if 'depth_image' in sample and \
                    not isinstance(sample['depth_image'], np.ndarray):
                raise

        sample['vis_image'] = vis_im
        sample['ir_image'] = ir_im
        if 'depth_image' in sample:
            if 'depth_im' not in locals():
                depth_im = sample['depth_image']
                if depth_im.ndim == 2:
                    depth_im = depth_im[..., None]
                depth_valid = (depth_im > 0).astype(np.float32)
            if depth_im.shape[:2] != vis_im.shape[:2]:
                raise ValueError(
                    'Depth and VIS shapes must match, got {} vs {}.'.format(
                        depth_im.shape[:2], vis_im.shape[:2]))
            sample['depth_image'] = depth_im.astype(np.float32)
            sample['depth_valid_mask'] = depth_valid.astype(np.float32)
        if 'h' not in sample:
...
class Multi_Permute(BaseOperator):
    ...
    def apply(self, sample, context=None):
        ...
        if 'depth_image' in sample:
            sample['depth_image'] = sample['depth_image'].transpose((2, 0, 1))
            sample['depth_valid_mask'] = sample[
                'depth_valid_mask'].transpose((2, 0, 1))
...
@register_op
class Multi_IndependentPhotometricDistort(BaseOperator):
    """Apply modest and independent photometric changes to RGB and IR.

    RGB receives brightness/contrast/saturation/hue perturbations. IR receives
    only brightness/contrast/gamma perturbations because hue and saturation do
    not have a stable physical meaning for the infrared stream. Depth and its
    valid mask are intentionally untouched.
    """
    ...
    def __init__(self,
                 ...
                 apply_prob=0.5,
                 rgb_brightness=16.0,
                 rgb_contrast=(0.85, 1.15),
                 rgb_saturation=(0.85, 1.15),
                 rgb_hue=6.0,
                 ir_brightness=12.0,
                 ir_contrast=(0.88, 1.12),
                 ir_gamma=(0.90, 1.10)):
        super(Multi_IndependentPhotometricDistort, self).__init__()
        self.prob = float(prob)
        self.apply_prob = float(apply_prob)
        self.rgb_brightness = float(rgb_brightness)
        self.rgb_contrast = rgb_contrast
        self.rgb_saturation = rgb_saturation
        self.rgb_hue = float(rgb_hue)
        self.ir_brightness = float(ir_brightness)
        self.ir_contrast = ir_contrast
        self.ir_gamma = ir_gamma

    def _coin(self):
        return np.random.uniform(0.0, 1.0) < self.apply_prob

    @staticmethod
    def _contrast(image, limits):
        factor = np.random.uniform(limits[0], limits[1])
        mean = image.mean(axis=(0, 1), keepdims=True)
        return (image - mean) * factor + mean

    def _distort_rgb(self, image):
        image = image.astype(np.float32)
        if self._coin():
            image += np.random.uniform(-self.rgb_brightness,
                                       self.rgb_brightness)
        if self._coin():
            image = self._contrast(image, self.rgb_contrast)
        if self._coin():
            factor = np.random.uniform(self.rgb_saturation[0],
                                       self.rgb_saturation[1])
            gray = np.sum(
                image * np.array([0.299, 0.587, 0.114], dtype=np.float32),
                axis=2,
                keepdims=True)
            image = gray + factor * (image - gray)
        if self._coin() and self.rgb_hue > 0:
            clipped = np.clip(image, 0.0, 255.0).astype(np.uint8)
            hsv = cv2.cvtColor(clipped, cv2.COLOR_RGB2HSV)
            hue_delta = np.random.uniform(-self.rgb_hue,
                                          self.rgb_hue) / 2.0
            hsv[..., 0] = np.mod(
                hsv[..., 0].astype(np.float32) + hue_delta, 180).astype(
                    np.uint8)
            image = cv2.cvtColor(hsv, cv2.COLOR_HSV2RGB).astype(np.float32)
        return np.clip(image, 0.0, 255.0)

    def _distort_ir(self, image):
        image = image.astype(np.float32)
        if self._coin():
            image += np.random.uniform(-self.ir_brightness,
                                       self.ir_brightness)
        if self._coin():
            image = self._contrast(image, self.ir_contrast)
        if self._coin():
            gamma = np.random.uniform(self.ir_gamma[0], self.ir_gamma[1])
            image = 255.0 * np.power(
                np.clip(image, 0.0, 255.0) / 255.0, gamma)
        return np.clip(image, 0.0, 255.0)

    def apply(self, sample, context=None):
        if np.random.uniform(0.0, 1.0) >= self.prob:
            return sample
        sample['vis_image'] = self._distort_rgb(sample['vis_image'])
        sample['ir_image'] = self._distort_ir(sample['ir_image'])
        return sample


@register_op
class Multi_RandomDepthInvalidation(BaseOperator):
    """Simulate sparse depth failures without changing RGB, IR or bboxes.

    Each sampled rectangle is cleared in both the normalized depth tensor and
    its valid mask.  This teaches the quality gate to rely on the mask instead
    of treating an interpolated or missing depth value as real geometry.
    """

    def __init__(self,
                 prob=0.3,
                 num_holes=(1, 4),
                 area_ratio=(0.005, 0.03),
                 aspect_ratio=(0.3, 3.0)):
        super(Multi_RandomDepthInvalidation, self).__init__()
        self.prob = float(prob)
        self.num_holes = tuple(num_holes)
        self.area_ratio = tuple(area_ratio)
        self.aspect_ratio = tuple(aspect_ratio)
        if len(self.num_holes) != 2 or self.num_holes[0] < 1 or \
                self.num_holes[0] > self.num_holes[1]:
            raise ValueError('num_holes must be a positive [min, max] pair.')
        if len(self.area_ratio) != 2 or self.area_ratio[0] <= 0 or \
                self.area_ratio[0] > self.area_ratio[1]:
            raise ValueError('area_ratio must be a positive [min, max] pair.')
        if len(self.aspect_ratio) != 2 or self.aspect_ratio[0] <= 0 or \
                self.aspect_ratio[0] > self.aspect_ratio[1]:
            raise ValueError(
                'aspect_ratio must be a positive [min, max] pair.')

    def apply(self, sample, context=None):
        if 'depth_image' not in sample or \
                np.random.uniform(0.0, 1.0) >= self.prob:
            return sample
        depth = sample['depth_image'].copy()
        valid = sample['depth_valid_mask'].copy()
        height, width = depth.shape[:2]
        image_area = float(height * width)
        holes = np.random.randint(self.num_holes[0], self.num_holes[1] + 1)
        for _ in range(holes):
            target_area = np.random.uniform(self.area_ratio[0],
                                            self.area_ratio[1]) * image_area
            aspect = np.random.uniform(self.aspect_ratio[0],
                                       self.aspect_ratio[1])
            hole_w = min(width, max(1, int(round(math.sqrt(target_area * aspect)))))
            hole_h = min(height, max(1, int(round(math.sqrt(target_area / aspect)))))
            x0 = np.random.randint(0, width - hole_w + 1)
            y0 = np.random.randint(0, height - hole_h + 1)
            depth[y0:y0 + hole_h, x0:x0 + hole_w] = 0.0
            valid[y0:y0 + hole_h, x0:x0 + hole_w] = 0.0
        sample['depth_image'] = depth
        sample['depth_valid_mask'] = valid
        return sample


@register_op
class Multi_RandomModalityDropout(BaseOperator):
    """Drop at most one modality so the detector remains useful when it fails.

    The probabilities are conditional weights once dropout is activated.  RGB
    and IR are replaced with their padding means, while Depth is invalidated by
    clearing both the value tensor and valid mask.  Dropping only one stream at
    a time avoids turning training samples into an ambiguous detection task.
    """

    def __init__(self,
                 prob=0.12,
                 modality_weights=(0.25, 0.25, 0.50),
                 fill_value=(123.675, 116.28, 103.53),
                 fill_value_ir=(114.495, 114.495, 114.495)):
        super(Multi_RandomModalityDropout, self).__init__()
        self.prob = float(prob)
        weights = np.asarray(modality_weights, dtype=np.float64)
        if weights.shape != (3, ) or np.any(weights < 0) or weights.sum() <= 0:
            raise ValueError('modality_weights must contain three non-negative values.')
        self.modality_weights = weights / weights.sum()
        self.fill_value = np.asarray(fill_value, dtype=np.float32)
        self.fill_value_ir = np.asarray(fill_value_ir, dtype=np.float32)

    def apply(self, sample, context=None):
        if np.random.uniform(0.0, 1.0) >= self.prob:
            return sample
        modality = np.random.choice(3, p=self.modality_weights)
        if modality == 0:
            sample['vis_image'] = np.empty_like(
                sample['vis_image'], dtype=np.float32)
            sample['vis_image'][...] = self.fill_value
        elif modality == 1:
            sample['ir_image'] = np.empty_like(
                sample['ir_image'], dtype=np.float32)
            sample['ir_image'][...] = self.fill_value_ir
        elif 'depth_image' in sample:
            sample['depth_image'] = np.zeros_like(sample['depth_image'],
                                                  dtype=np.float32)
            sample['depth_valid_mask'] = np.zeros_like(
                sample['depth_valid_mask'], dtype=np.float32)
        return sample


@register_op
class Multi_RandomAffine(BaseOperator):
    """Synchronously rotate/scale/translate RGB, IR, Depth, mask and bboxes."""
    ...
    def __init__(self,
                 ...
                 prob=0.5,
                 min_bbox_size=2.0,
                 min_bbox_area_ratio=0.20,
                 fill_value=(123.675, 116.28, 103.53),
                 fill_value_ir=(114.495, 114.495, 114.495)):
        super(Multi_RandomAffine, self).__init__()
        ...
        self.fill_value_ir = tuple(fill_value_ir)
        if len(scale) != 2 or scale[0] <= 0 or scale[0] > scale[1]:
            raise ValueError('scale must be a positive [min, max] pair.')

    def _sample_matrix(self, width, height):
        angle = np.random.uniform(-self.degrees, self.degrees)
        scale = np.random.uniform(self.scale[0], self.scale[1])
        center = ((width - 1.0) * 0.5, (height - 1.0) * 0.5)
        matrix = cv2.getRotationMatrix2D(center, angle, scale)
        matrix[0, 2] += np.random.uniform(-self.translate,
                                          self.translate) * width
        matrix[1, 2] += np.random.uniform(-self.translate,
                                          self.translate) * height
        return matrix.astype(np.float32)

    @staticmethod
    def _warp_image(image, matrix, width, height, fill_value):
    ...
    @staticmethod
    def _warp_depth(depth, valid, matrix, width, height):
        depth_2d = depth.reshape(depth.shape[0], depth.shape[1])
        valid_2d = valid.reshape(valid.shape[0], valid.shape[1]).astype(
            np.float32)
        ...
        warped_valid = cv2.warpAffine(
            ...
            borderMode=cv2.BORDER_CONSTANT,
            borderValue=0.0)
        warped_valid = (warped_valid > 0.5).astype(np.float32)
        warped_depth = np.divide(
            weighted,
            np.maximum(support, 1e-6),
            out=np.zeros_like(weighted),
            where=support > 1e-6)
        warped_depth *= warped_valid
        return warped_depth[..., None], warped_valid[..., None]

    def _transform_bboxes(self, bboxes, matrix, width, height):
        if len(bboxes) == 0:
            return bboxes, np.zeros((0, ), dtype=np.int64)
        corners = np.stack([
            bboxes[:, [0, 1]], bboxes[:, [2, 1]], bboxes[:, [2, 3]],
            bboxes[:, [0, 3]]
        ],
                           axis=1)
        homogeneous = np.concatenate(
            [corners, np.ones((*corners.shape[:2], 1), dtype=np.float32)],
            axis=2)
        transformed = np.matmul(homogeneous, matrix.T)
        new_boxes = np.concatenate(
            [transformed.min(axis=1), transformed.max(axis=1)], axis=1)
        new_boxes[:, 0::2] = np.clip(new_boxes[:, 0::2], 0.0, width)
        new_boxes[:, 1::2] = np.clip(new_boxes[:, 1::2], 0.0, height)

        old_area = ((bboxes[:, 2] - bboxes[:, 0]) *
                    (bboxes[:, 3] - bboxes[:, 1]))
        area_scale = abs(np.linalg.det(matrix[:, :2]))
        new_width = new_boxes[:, 2] - new_boxes[:, 0]
        new_height = new_boxes[:, 3] - new_boxes[:, 1]
        new_area = new_width * new_height
        keep = np.logical_and.reduce([
            new_width >= self.min_bbox_size,
            new_height >= self.min_bbox_size,
            new_area >= old_area * area_scale * self.min_bbox_area_ratio
        ])
        keep_ids = np.nonzero(keep)[0]
        return new_boxes[keep_ids].astype(np.float32), keep_ids

    @staticmethod
    def _filter_instances(sample, keep_ids):
        for key in [
                'gt_class', 'is_crowd', 'gt_score', 'difficult', 'gt_areas',
                'gt_track_id'
        ]:
            if key in sample:
                sample[key] = np.take(sample[key], keep_ids, axis=0)
        for key in ['gt_poly', 'gt_segm']:
            if key in sample:
                if isinstance(sample[key], np.ndarray):
                    sample[key] = np.take(sample[key], keep_ids, axis=0)
                else:
                    sample[key] = [sample[key][i] for i in keep_ids]

    def apply(self, sample, context=None):
        if np.random.uniform(0.0, 1.0) >= self.prob:
            return sample
        height, width = sample['vis_image'].shape[:2]
        matrix = self._sample_matrix(width, height)
        sample['vis_image'] = self._warp_image(
            sample['vis_image'], matrix, width, height, self.fill_value)
        sample['ir_image'] = self._warp_image(
            sample['ir_image'], matrix, width, height, self.fill_value_ir)
        if 'depth_image' in sample:
            depth, valid = self._warp_depth(sample['depth_image'],
                                            sample['depth_valid_mask'], matrix,
                                            width, height)
            sample['depth_image'] = depth.astype(np.float32)
            sample['depth_valid_mask'] = valid.astype(np.float32)
        if 'gt_bbox' in sample:
            bboxes, keep_ids = self._transform_bboxes(
                sample['gt_bbox'], matrix, width, height)
            sample['gt_bbox'] = bboxes
            self._filter_instances(sample, keep_ids)
            if 'gt_areas' in sample:
                sample['gt_areas'] = (
                    (bboxes[:, 2] - bboxes[:, 0]) *
                    (bboxes[:, 3] - bboxes[:, 1])).astype(np.float32)
        return sample
...
class Multi_RandomFlip(BaseOperator):
    ...
    def apply(self, sample, context=None):
        ...
        if np.random.uniform(0, 1) < self.prob:
            ...
            if 'depth_image' in sample:
                sample['depth_image'] = self.apply_image(
                    sample['depth_image'])
                sample['depth_valid_mask'] = self.apply_image(
...
class Multi_Resize(BaseOperator):
    ...
    @staticmethod
    def apply_depth(depth, valid, size):
        """Mask-aware bilinear resize without bleeding invalid zeros."""
        resize_w, resize_h = size
        depth_2d = depth.reshape(depth.shape[0], depth.shape[1])
        valid_2d = valid.reshape(valid.shape[0], valid.shape[1]).astype(
            np.float32)
        weighted = cv2.resize(
            depth_2d * valid_2d, (resize_w, resize_h),
            interpolation=cv2.INTER_LINEAR)
        support = cv2.resize(
            valid_2d, (resize_w, resize_h), interpolation=cv2.INTER_LINEAR)
        resized_valid = cv2.resize(
            valid_2d, (resize_w, resize_h), interpolation=cv2.INTER_NEAREST)
        resized_valid = (resized_valid > 0.5).astype(np.float32)
        resized_depth = np.divide(
            weighted,
            np.maximum(support, 1e-6),
            out=np.zeros_like(weighted),
            where=support > 1e-6)
        resized_depth *= resized_valid
        return resized_depth[..., None], resized_valid[..., None]
    ...
    def apply(self, sample, context=None):
        ...
        if len(vis_im.shape) == 3:
            ...
            if 'depth_image' in sample:
                depth, valid = self.apply_depth(
                    sample['depth_image'], sample['depth_valid_mask'],
                    (resize_w, resize_h))
                sample['depth_image'] = depth.astype(np.float32)
                sample['depth_valid_mask'] = valid.astype(np.float32)
        else:
    ...
    def random_crop(self, sample, fake_bboxes=False):
        ...
        for thresh in thresholds:
            ...
            if found:
                ...
                if 'depth_image' in sample:
                    sample['depth_image'] = self._crop_image(
                        sample['depth_image'], crop_box)
                    sample['depth_valid_mask'] = self._crop_image(
                        sample['depth_valid_mask'], crop_box)
                if fake_bboxes == True:
                    return sample
...
class Multi_Pad(BaseOperator):
    ...
    def apply(self, sample, context=None):
        ...
        if h == im_h and w == im_w:
            ...
            sample['ir_image'] = ir_im.astype(np.float32)
            if 'depth_image' in sample:
                sample['depth_image'] = sample['depth_image'].astype(np.float32)
                sample['depth_valid_mask'] = sample[
                    'depth_valid_mask'].astype(np.float32)
            return sample
...
class Multi_Pad_IRVIS(BaseOperator):
    def __init__(self,
        ...
        self.fill_value_ir = fill_value_ir
        self.offsets = offsets
        self.return_pad_mask = bool(return_pad_mask)
    ...
    def apply(self, sample, context=None):
        ...
        if h == im_h and w == im_w:
            ...
            if self.return_pad_mask:
                sample['pad_mask'] = np.ones((h, w), dtype=np.float32)
            return sample
        ...
        if 'depth_image' in sample:
            x, y = offsets
            depth_canvas = np.zeros((h, w, 1), dtype=np.float32)
            valid_canvas = np.zeros((h, w, 1), dtype=np.float32)
            depth_canvas[y:y + im_h, x:x + im_w] = sample[
                'depth_image'].reshape(im_h, im_w, 1)
            valid_canvas[y:y + im_h, x:x + im_w] = sample[
                'depth_valid_mask'].reshape(im_h, im_w, 1)
            sample['depth_image'] = depth_canvas
            sample['depth_valid_mask'] = valid_canvas

        if self.return_pad_mask:
            pad_mask = np.zeros((h, w), dtype=np.float32)
            pad_mask[offset_y:offset_y + im_h,
                     offset_x:offset_x + im_w] = 1.0
