...
class Mutisprctral_COCODataSet(Multi_DetDataset):
    ...
    def parse_dataset(self):
        ...
        depth_image_dir = os.path.join(self.dataset_dir, self.depth_image_dir) \
            if self.depth_image_dir else None
        ...
        for img_id in img_ids:
            ...
            depth_im_path = os.path.join(depth_image_dir, im_fname) \
                if depth_image_dir else None
            is_empty = False
            ...
            if not os.path.exists(ir_im_path):
                ...
                continue
            if depth_im_path is not None and not os.path.exists(depth_im_path):
                logger.warning('Illegal depth image file: {}, and it will be '
                               'ignored'.format(depth_im_path))
                continue
            ...
            if depth_im_path is not None and 'image' in self.data_fields:
                coco_rec['depth_im_file'] = depth_im_path
            ...
            logger.debug(
                'Load vis_file: {}, ir_file: {}, depth_file: {}, im_id: {}, '
                'h: {}, w: {}.'.format(vis_im_path, ir_im_path,
                                       depth_im_path, img_id, im_h, im_w))
            if is_empty:
