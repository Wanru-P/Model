...
def run(FLAGS, cfg):
    ...
    # Formal ablation experiments can opt into a shared deterministic seed from
    # YAML.  Keep the historical continuous-evaluation behaviour unchanged.
    if FLAGS.enable_ce:
        set_random_seed(0)
    elif cfg.get('seed', None) is not None:
        set_random_seed(int(cfg.seed))
        logger.info("Random seed is fixed to {}.".format(int(cfg.seed)))
